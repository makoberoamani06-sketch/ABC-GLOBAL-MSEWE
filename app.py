...
ADMIN_PASS = os.environ.get('ADMIN_PASS', 'MSEWE123')
# Make the admin path configurable and secret. Set ADMIN_PATH in env (e.g. ADMIN_PATH=msewe-admin)
ADMIN_PATH = os.environ.get('ADMIN_PATH', 'msewe-admin')
...
# Pages: keep serving static files for main site
@app.route('/')
def index():
...
# Hidden admin routes: paths are based on ADMIN_PATH (configurable via env)
...
@app.route(f"{ADMIN_PREFIX}/registration/<int:reg_id>/delete", methods=['POST'])
@admin_required
...
@app.route(f"{ADMIN_PREFIX}/contact/<int:c_id>/delete", methods=['POST'])
@admin_required
...
@app.route(f"{ADMIN_PREFIX}/export/registrations")
@admin_required
...
@app.route(f"{ADMIN_PREFIX}/export/contacts")
@admin_required
def export_contacts():
    import csv
    from io import StringIO
...
# Redirect the public /admin paths to index to avoid exposing the admin area
@app.route('/admin')
def admin_public_root():
    return redirect(url_for('index'))

@app.route('/admin/<path:rest>')
def admin_public_catchall(rest):
    # Hide default admin endpoints by redirecting public access to the homepage
    return redirect(url_for('index'))
